...
function StatCard({
  ...
  return (
    <div
      className={`rounded-2xl p-5 shadow-sm border flex flex-col justify-between h-28 ${highlight ? "bg-red-500 border-red-500" : "bg-white border-gray-100"}`}
    >
      <p
        className={`text-xs font-medium ${highlight ? "text-red-100" : "text-gray-400"}`}
      >
        {label}
      </p>
      <p
        className={`text-3xl font-bold ${highlight ? "text-white" : "text-gray-800"}`}
      >
        {value}
      </p>
    </div>
...
export default function Dashboard() {
  ...
  const clusterKeys =
    trend.length > 0 ? Object.keys(trend[0]).filter((k) => k !== "month") : [];
  const trendColors: Record<string, string> = {};
  clusterKeys.forEach((k, i) => {
    trendColors[k] = CLUSTER_COLORS[i % CLUSTER_COLORS.length];
  });
            ...
            {clusters.map((c, i) => (
              ...
              >
                <div
                  ...
                  style={{
                    height: `${(c.total_calls / maxCalls) * 100}%`,
                    backgroundColor: CLUSTER_COLORS[i % CLUSTER_COLORS.length],
                  }}
                />
              ...
              <div className="flex items-center gap-3 mb-5 p-3 bg-gray-50 rounded-xl">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-800">
                    {recentCall.agent?.name ?? "—"}
                  </p>
                  <p className="text-xs text-gray-400">
                    Agent · {recentCall.cluster?.name ?? "—"}
                  </p>
                </div>
                {recentCall.analysis && (
                  <span
                    className={`ml-auto px-2 py-1 text-xs font-semibold rounded-full ${
                      recentCall.analysis.risk_level === "Critical"
                        ? "bg-red-100 text-red-700"
                        : recentCall.analysis.risk_level === "High"
                          ? "bg-orange-100 text-orange-700"
                          : recentCall.analysis.risk_level === "Medium"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 rounded-xl p-4 text-center">
                  ...
                  <p className="text-base font-bold text-gray-800">
                    {recentCall.call_date
                      ? new Date(recentCall.call_date).toLocaleDateString(
                          "en-US",
                ...
                <div className="bg-red-50 rounded-xl p-4 text-center">
                  ...
                  <p className="text-base font-bold text-red-600">
                    {recentCall.analysis?.risk_level ?? "—"}
                  </p>
                </div>
              </div>
            </>
