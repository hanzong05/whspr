...
export default function AgentsPage() {
  ...
  const fetchAgents = async () => {
    ...
    try {
      ...
      const data: AgentRow[] = await fetch(`${API}/agents${params}`).then((r) =>
        r.json(),
      );
      setAgents(data);
    } catch {
        ...
        <Modal onClose={() => setViewTarget(null)} maxWidth="sm">
          ...
          <Modal.Body>
            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
              <div className="w-14 h-14 rounded-full bg-red-100 text-red-600 text-xl font-bold flex items-center justify-center flex-shrink-0">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                ...
                <span
                  className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${riskConfig[viewTarget.risk_level].badge}`}
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${riskConfig[viewTarget.risk_level].dot}`}
                  />
            ...
            {loading ? (
              ...
              filtered.map((agent) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1 max-w-16 h-1.5 bg-gray-100 rounded-full">
                        <div
                          ...
                          style={{
                            width: `${Math.min((agent.total_calls / 500) * 100, 100)}%`,
                          }}
                        />
                      </div>
                    </div>
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${riskConfig[agent.risk_level].badge}`}
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${riskConfig[agent.risk_level].dot}`}
                      />
                      {agent.risk_level}
                    </span>
                  </td>
